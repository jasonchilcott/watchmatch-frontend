...
class MatchCard extends React.Component {
  ...
  <div className="match-card-section">{this.colorCompatibility()}</div>
  ...
  <div className="match-card-section">
    <div className="match-info">
      ...
      >
        ...
        {this.props.match.ratings ? (
          <h4 className="match-ratings-number">{`${this.props.match.ratings.length} ratings`}</h4>
        ) : null}
      </NavLink>
      {this.props.match.one_line ? (
        <p className="match-one-line">{this.props.match.one_line}</p>
      ) : (
        <p>This user hasn't added a one-liner yet.</p>
      )}
    </div>
  </div>
</div>
